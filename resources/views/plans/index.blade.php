    ...
    <div class="row" id="packagelist">
        @if($plans)
            @foreach($plans as $plan)
                <div class="col-sm-4">
                    @php $user= auth()->user(); if (auth()->user()->type !='admin'){
                                                        $userPlan=\App\Models\UserPlan::where('user_id',auth()->user()->id)->where('is_current','yes')->first();
                    ...
                    <div
                        ...
                        @if(isset($userPlan) && $userPlan->plan_id == $plan->id && $userPlan->subscription_id)
                            <a href="{{route('cancel.subscription',['type'=>$userPlan->payment_type])}}"
                               class="btn-sm btn badge-danger light"
                               data-user-plan="{{auth()->user()->current_plans[0]->plan_id}}" id="subscriptionCancel">Cancel</a>
                        @endif
