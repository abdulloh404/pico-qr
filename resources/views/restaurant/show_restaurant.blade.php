                                                ...
                                                <div class="price">
                                                    @if($item->discount>0)
                                                        @if($item->discount_type=='percent')
                                                            @php $discount = $item->discount*$item->price /100; $total= $item->price - $discount ; @endphp
                                                            <del>{{isset($restaurant->currency_symbol)? $restaurant->currency_symbol.''.$item->price:formatNumberWithCurrSymbol($item->price)}}</del> {{isset($restaurant->currency_symbol)? $restaurant->currency_symbol.''.$total :formatNumberWithCurrSymbol(($item->price-(($item->discount*$item->price)/100)))}}
                                                        @elseif($item->discount_type=='flat')
                                                            <del>{{isset($restaurant->currency_symbol)? $restaurant->currency_symbol.''.$item->price:formatNumberWithCurrSymbol($item->price)}}</del>
                                                            {{isset($restaurant->currency_symbol)? $restaurant->currency_symbol.''.($item->price-$item->discount):formatNumberWithCurrSymbol($item->price-$item->discount)}}
                                                        @endif
                                                    @else
                                                    ...
                                                    <div class="text-right">
                                                        @if(isset($order) && $order->payment_status !='paid')
                                                            <button data-item-name="{{$item->name}}"
                                                                    ...
                                                                    class="btn btn-xxs btn-info update-order">{{trans('layout.add')}}
                                                            </button>
                                                        @else
                                                            <button
                                                                data-extras="{{json_encode($item->active_extras)}}"
                                ...
                                <div class="order-items-payment">
                                    <div class="row">
                                        ...
                                        <div class="col-md-12 col-12 mb-3 ">
                                            <label for="table_id">{{trans('layout.select_delivery_type')}}</label>
                                        ...
                                        <div id="takeawayTime" class="col-sm-12 mt-3 takeaway-section d-none">
                    ...
                    <div class="card">
                        <div class="card-header row pb-0">
                            <h5 class="card-title col-sm-8 col-8">{{trans('layout.pay_now')}} </h5> <span
                                class="float-right text-muted col-sm-4 col-4">{{trans('layout.total')}}: {{isset($restaurant->currency_symbol) && isset($order->total_price)? $restaurant->currency_symbol.''.$order->total_price:formatNumberWithCurrSymbol(isset($order->total_price)?$order->total_price:0)}}</span>
                            <hr>
                        </div>
                        <div class="card-body">
    ...
    @if(isset($orderStatus->admin_order_status) && $orderStatus->admin_order_status=='enable' && isset($restaurant->order_status) && $restaurant->order_status=='enable')
        <div class="row  view-order-section">
            @if(isset($order) && $order->status !='delivered')
                <button
                    class="btn btn-sm badge-success light view-order-btn small-view-order-btn">{{trans('layout.view_order')}}</button>
            @elseif(isset($order) && $order->status =='delivered' && $order->payment_status=='unpaid')
                <button
                    class="btn btn-sm badge-success light view-order-btn small-view-order-btn_t">{{trans('layout.view_order')}}</button>
                <button
...
@section('js')
    ...
    <script>
                            ...
                            html += `<div class="row">
                                   ...
                                   <div class="col-sm-6 col-6 text-right">
                                       <div class="modify-item">
                                           ...
                                           <div id="quantityMinus" data-details-id="${value.id}" class="minus-quantity">
                                               <i class="fa fa-minus"></i>
                                           </div>
                                           <div id="item-quantity" class="item-quantity">${value.quantity}</div>
                                           <div id="quantityPlus" data-details-id="${value.id}"  class="plus-quantity" id="plus-quantity-1">
                                               <i class="fa fa-plus"></i>
                                           </div>
                                       </div>
                                   </div>
        ...
        $(document).on('click', '#quantityPlus', function (e) {
            ...
            const detailsId = $(this).attr('data-details-id');
            ...
            $('#saveOrder').attr('data-details-id', detailsId);
