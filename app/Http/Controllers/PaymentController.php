...
class PaymentController extends Controller
{
    public function payment(Request $request)
    {
        ...
        if ($plan->cost != 0) {
            if (isset($paymentSetting->stripe_secret_key) && isset($paymentSetting->stripe_status) && $paymentSetting->stripe_status == 'active') {
    ...
    public function process(Request $request)
    {
        ...
        if ($request->payment_type == 'paypal') {
            ...
            try {
                $payment = $this->stripePayment($plan, $request, $userPlan);
    ...
    function stripePayment($plan, $req, $userPlan)
    {
        $authUser = auth()->user();
        $credentials = json_decode(get_settings('payment_gateway'));
...
//        User Plan
        $userPlan = UserPlan::where('user_id', $authUser->id)->first();
//        Customer
...
//        Price
        $price = $stripe->prices->create([
            ...
            'currency' => 'usd',
            'recurring' => ['interval' => $recurring_type],
            'product' => $product->id,
        ]);
...
*/
        ...
        $userPlan->subscription_id = $paytmUserInfo['custId'];
        $userPlan->payment_type = 'paytm';
        $userPlan->save();
    ...
    public function subscriptionCancel(Request $request)
    {
        ...
        $userPlan = UserPlan::where('user_id', $authUser->id)->where('is_current', 'yes')->first();
        if ($request->type == 'stripe') {
            ...
            $this->paytmSubscriptionCancel($userPlan);
        }
        $userPlan->subscription_id = '';
        $userPlan->save();

        return redirect()->back()->with('success', trans('Successfully canceled subscription'));
    }


    function paytmSubscriptionCancel($userPlan)
    {
        ...
        $paytmParams["body"] = array(
            "mid" => $credentials->paytm_mid,
            "subsId" => $userPlan->subscribtion_id,
        );
